      ...
      if import_urls
        ...
        import_urls.each do |import_url|
          ...
          begin
            ...
            archive_result = Net::HTTP.post_form(source_archive_uri, {}).body
          rescue => ex
            error_projects.append new_project_name
            logger.error {"GitLab Project Generater: Project '#{new_project_name}' has error: #{ex.message}, Archive url: #{source_archive_uri if source_archive_uri}, Archive result: #{archive_result if archive_result}"}
          end
        end
